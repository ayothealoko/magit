...
function Level1NonCollaspable(props:Level1NonCollaspableProps): JSX.Element{
    ...
    return(
	<>
	    return <Line> <StyledSpan {...props} /> </Line>
	</>
   ) 
}
...
function Level1Collaspable({text, bgColor, color, level2}:Level1CollaspableProps): JSX.Element{
    const [isOpen, setIsOpen] = useState(true);
    const handleClickToggle = () => {
	setIsOpen(val => !val);
    }
...
function Level2({data}:Level2Props): JSX.Element{
	...
	<>
	{data.map((t,i) => {
	    return <Line key={i}> <StyledSpan {...t} /> </Line>
	})}
	</>
   ) 
}
